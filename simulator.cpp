...
void P2P::compute_statistics() {
    map<int, int> blocks_in_longest_chain;
    map<int, int> total_blocks_generated;

    // Count total blocks generated by each node
    for (auto &peer : peers) {
        total_blocks_generated[peer.peerID] = peer.total_blocks_generated;
    }
    
    // Traverse the longest chain and count blocks per node
    int longestChainBlock = -1;
    int maxDepth = 0;

    // Find the block at the end of the longest chain
    for (auto &peer : peers) {
        if (peer.maxDepth > maxDepth) {
            maxDepth = peer.maxDepth;
            longestChainBlock = peer.longestChain;
        }
    }
    ...
    for (auto &peer : peers) {
        if (peer.blockTree.count(longestChainBlock)) {
            currNode = peer.blockTree[longestChainBlock];
            break;
        }
    }

    while (currNode != nullptr) {
        int blockID = currNode->block_id;
        for (auto &peer : peers) {
            if (peer.blockSet.count(blockID)) {
                blocks_in_longest_chain[peer.peerID]++;
                break;
            }
        }
    ...
    // Step 3: Compute and display ratios
    std::cout << "PeerID, Blocks in Longest Chain, Total Blocks, Ratio (R_i), Type\n";
    for (auto &peer : peers) {
        int inLongestChain = blocks_in_longest_chain[peer.peerID];
        int totalGenerated = total_blocks_generated[peer.peerID];
        double ratio = totalGenerated > 0 ? (double)inLongestChain / totalGenerated : 0.0;

        std::string type = (peer.slow ? "Slow" : "Fast");
        type += peer.lowCPU ? " Low-CPU" : " High-CPU";

        std::cout << peer.peerID << ", " << inLongestChain << ", " << totalGenerated << ", " 
                  << ratio << ", " << type << "\n";
    }
}
...
int main(int argc, char **argv)
{
    ...
    P2P *simulator = new P2P(stoi(argv[1]), stoi(argv[2]), stoi(argv[3]), stoi(argv[4]), stoi(argv[5]), stoi(argv[6]));
